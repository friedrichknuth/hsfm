...
def dem_align_custom(
    ...
    if print_call == True:
        ...
        log_file = run_command(
            call, verbose=verbose, log_directory=dem_align_output_path
        )
        try:
            dem_difference_file = glob.glob(
                os.path.join(dem_align_output_path, "*_align_diff.tif")
            )[0]
            aligned_dem_file = glob.glob(
                os.path.join(dem_align_output_path, "*_align.tif")
            )[0]

            return dem_difference_file, aligned_dem_file
        except:
            print(
                "Unable to align dem using dem_align.py. See",
                log_file,
                "for additional details.",
            )


def mask_dem(dem, output_directory=None, masks=["--nlcd", "--glaciers"], verbose=True):
    ...
    if not isinstance(output_directory, type(None)):
        ...
        if file_path != output_directory:
            output_file_name = os.path.join(
                output_directory, file_name + file_extension
            )
        else:
            output_file_name = os.path.join(
                file_path, file_name + "_sub" + str(scale) + file_extension
            )
...
def optimize_geotif(geotif_file_name, output_file_name=None, verbose=False):
...
def download_srtm(
    ...
    ds = gdal.Translate(
        vrt_subset_file_name, ds, projWin=[bounds[0], bounds[1], bounds[2], bounds[3]]
    )
    ...
    if utm:
        ...
        call = (
            "gdalwarp -co COMPRESS=LZW -co TILED=YES -co BIGTIFF=IF_SAFER -dstnodata -9999 -r cubic -t_srs EPSG:"
            + epsg_code
        )
        call = call.split()
        ...
        if cleanup == True:
            out = os.path.join(
                output_directory, os.path.split(utm_vrt_subset_file_name)[-1]
            )
        ...
        if cleanup == True:
            out = os.path.join(
                output_directory, os.path.split(adjusted_vrt_subset_file_name)[-1]
            )
...
def clip_reference_dem(
    ...
    center = str(
        rasterio_dataset.xy(rasterio_dataset.height // 2, rasterio_dataset.width // 2)
    )
...
def difference_dems(dem_file_name_a, dem_file_name_b, verbose=False):
...
def pick_headings(image_directory, camera_positions_file_name, subset, delta=0.015):
    ...
    for i, v in enumerate(lats):
        ...
        heading = pick_heading_from_map(
            image_file_name, camera_center_lon, camera_center_lat, dx=delta, dy=delta
        )
...
def pick_camera_location(image_file_path, center_lon, center_lat, dx=0.030, dy=0.030):
    ...
    extents = (center_lon - dx, center_lat - dy, center_lon + dx, center_lat + dy)

    # run the tile server
    tiles = gv.WMTS(url, extents=extents)

    points = gv.Points(
        [(center_lon, center_lat, "starting_center")], vdims="image_file_name"
    )
    ...
    projected = gv.operation.project_points(
        point_stream.element, projection=ccrs.PlateCarree()
    )
...
def pick_heading_from_map(
    image_file_name, camera_center_lon, camera_center_lat, dx=0.015, dy=0.015
):
    ...
    points = gv.Points(
        [(camera_center_lon, camera_center_lat, "camera_center")], vdims="location"
    )
    ...
    heading = hsfm.geospatial.calculate_heading(
        camera_center_lon, camera_center_lat, heading_lon, heading_lat
    )
...
## TODO move to hsfm.tools (needs to be created) as this launches a self contained app
def create_fiducials(image_array, output_directory="fiducials"):
...
def launch_fiducial_picker(hv_image, subplot_width, subplot_height):
    ...
    principal_point = hsfm.core.determine_principal_point(
...
## TODO move to hsfm.core as best fit (for now)
def hv_plot_raster(image_file_name, stretch_histogram=False):
...
def pick_fiducials(image_file_name):
    ...
    fiducials, principal_point = hsfm.utils.launch_fiducial_picker(
        hv_image, subplot_width, subplot_height
    )
...
def run_command2(command, verbose=False, log=False):
    ...
    p = Popen(command, universal_newlines=True, stdout=PIPE, stderr=STDOUT, shell=True)
