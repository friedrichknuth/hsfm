...
def bundle_adjust_custom(
    ...
    input_camera_files = sorted(
        glob.glob(os.path.join(camera_files_directory, "*.tsai"))
    )
    ...
    if print_asp_call == True:
        ...
        hsfm.utils.run_command(call, verbose=verbose, log_directory=log_directory)
        ...
        if qc == True:
            ...
            try:
                ...
                bare.batch.plot_mp_over_images(
                    destination_file_path,
                    image_files_directory,
                    output_directory=os.path.join(output_directory, "qc/ba_matches"),
                )
            ...
            print(
                "camera_solve match point qc plots saved in",
                os.path.join(output_directory, "qc/ba_matches"),
            )

        return ba_output_directory


def parallel_stereo_custom(
    first_image,
    second_image,
    first_camera,
    second_camera,
    ...
    if print_asp_call == True:
        ...
        hsfm.utils.run_command(call, verbose=False, log_directory=log_directory)
        print("Parallel stereo results saved in", stereo_output_directory)
        return stereo_output_directory


def dem_mosaic_custom(output_directory, verbose=False, print_asp_call=False):
    """
...
def generate_match_points(
    ...
    call = ["camera_solve", output_directory]
            ...
            try:
                hsfm.io.batch_rename_files(
                    ...
                    destination_file_path=os.path.join(
                        qc_output_directory, "match_files/cam_solve/"
                    ),
                )
                clean_match_file_list = sorted(
                    glob.glob(
                        os.path.join(
                            qc_output_directory,
                ...
                bare.core.iter_mp_to_csv(qc_output_directory, "match_files/cam_solve/")
                try:
                    hsfm.batch.plot_match_overlap(
                        os.path.join(qc_output_directory, "match_files/cam_solve/"),
                        image_directory,
                        output_directory=os.path.join(
                            qc_output_directory, "cam_solve_matches"
                        ),
                    )
                except:
                    bare.batch.plot_mp_over_images(
                ...
                print(
                    "camera_solve match point qc plots saved in qc/cam_solve_matches/"
                )
...
def point2dem(point_cloud_file, *args, print_call=False, verbose=False):
...
def pc_align(
    ...
    *args,
    ...
    call.extend(args)
    call.extend([reference_dem_file, input_dem_file, "-o", output_directory_prefix])
    ...
    if print_call:
        ...
        if create_dem:
            ...
            aligned_dem_file = point2dem(point_cloud_file, "--t_srs", epsg_code)

            return aligned_dem_file, transform

        else:
            return point_cloud_file, transform
...
def pc_align_p2p_sp2p(
    ...
    aligned_dem_file, transform = hsfm.asp.pc_align(
        input_dem_file,
        reference_dem_file,
        output_directory,
...
def iter_stereo_pairs(
    ...
    input_camera_files = sorted(
        glob.glob(os.path.join(camera_files_directory, "*" + camera_extension))
    )
    ...
    for match_file in match_files:
        ...
        hsfm.io.rename_file(
            match_file, pattern="-clean", destination_file_path=output_dir, write=True
        )

        output_directory = os.path.join(
            stereo_output_directory, output_folder + "/asp_ba_out"
        )

        print("Running parallel stereo on", image_a, "and", image_b)

        stereo_run_output_directory = parallel_stereo_custom(
            image_a,
            image_b,
            camera_a,
            camera_b,
        ...
        try:
            point_cloud_file_name = glob.glob(
                os.path.join(stereo_run_output_directory, "*PC.tif")
            )[0]
            point2dem(point_cloud_file_name, "--t_srs", projection, "--errorimage")
        except:
            print(
                "Unable to generate point cloud from", match_file_a, "and", match_file_b
            )
...
def transform_cameras(
    ...
    output_directory = bundle_adjust(
        image_files,
        camera_files,
        output_directory_prefix,
        "-t",
