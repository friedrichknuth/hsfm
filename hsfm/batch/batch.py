...
def prepare_ba_run(input_directory, output_directory, scale):
    ...
    overlap_list = hsfm.core.create_overlap_list_from_match_files(
        camera_solve_directory, images_directory, output_directory
    )

    if not os.path.exists(os.path.join(bundle_adjust_directory, "overlaplist.txt")):
        gcp_directory = os.path.join(input_directory, "gcp")
        overlap_list = hsfm.core.create_overlap_list(
            gcp_directory, images_directory, output_directory=output_directory
        )
...
def rescale_images(
    image_directory, output_directory, extension=".tif", scale=8, verbose=False
):
    ...
    for image_file in image_files:
        ...
        hsfm.utils.rescale_geotif(
            image_file, output_directory=output_directory, scale=scale, verbose=verbose
        )
...
def rescale_tsai_cameras(
    camera_directory, output_directory, extension=".tsai", scale=8
):
    ...
    for camera_file in camera_files:
        ...
        output_file = os.path.join(
            output_directory, file_name + "_sub" + str(scale) + file_extension
        )
...
def batch_generate_cameras(
    ...
    if manual_heading_selection == False:
        ...
        df = hsfm.utils.pick_headings(
            image_directory, camera_positions_file_name, subset, delta=0.01
        )

    if len(image_list) != len(df):
        print(
            "Mismatch between metadata entries in camera position file and available images."
        )
        sys.exit(1)
    ...
    for i, v in enumerate(image_list):
        ...
        gcp_directory = hsfm.core.prep_and_generate_gcp(
            image_file_name,
            camera_lat_lon_center_coordinates,
            reference_dem_file_name,
            focal_length_mm,
    ...
    intial_cameras_directory = hsfm.core.initialize_cameras(
        camera_positions_file_name,
        reference_dem_file_name,
        focal_length_px,
        principal_point_px,
    ...
    output_directory = hsfm.asp.generate_ba_cameras(
        image_directory,
        gcp_directory,
...
def calculate_heading_from_metadata(
    df,
    subset=None,
    reverse_order=False,
    ...
    if for_metashape:
        ...
        if output_directory:
            hsfm.io.create_dir(output_directory)
            df.to_csv(
                os.path.join(output_directory, "metashape_metadata.csv"), index=False
            )
...
def NAGAP_pre_process_images(
    ...
    df = hipp.dataquery.NAGAP_pre_select_images(
        nagap_metadata_csv, bounds=bounds, roll=roll, year=year, month=month, day=day
    )
            ...
            for month in sorted(list(set(df_roll["Month"].values))):
                ...
                if len(list(set(df_month["Day"].values))) >= 1:
                    for day in sorted(list(set(df_month["Day"].values))):
                        ...
                        out_dir_day = os.path.join(
                            out_dir_month, str(int(day)).zfill(2)
                        )
                        df_day = df_month[df_month["Day"] == day].copy()
                        hsfm.batch.NAGAP_pre_process_set(
                            df_day,
                            template_types,
                            template_dirs,
                            out_dir_day,
                ...
                # in case no day specified in metadata
                    ...
                    out_dir_month = os.path.join(
                        output_directory, roll, str(int(month)).zfill(2), "dd"
                    )
                    hsfm.batch.NAGAP_pre_process_set(
                        df_month,
                        template_types,
        ...
        if not df_tmp.empty:
            if download_images == True:
                image_directory = hipp.dataquery.NAGAP_download_images_to_disk(
                    df_tmp,
                    output_directory=os.path.join(output_directory, v + "_raw_images"),
                )
                ...
                image_square_dim = hipp.batch.preprocess_with_fiducial_proxies(
                    ...
                    output_directory=os.path.join(
                        output_directory, v + "_cropped_images"
                    ),
                    missing_proxy=missing_proxy,
                    qc_df_output_directory=os.path.join(
                        output_directory, "qc", v + "_proxy_detection_data_frames"
                    ),
                    qc_plots_output_directory=os.path.join(
                        output_directory, "qc", v + "_proxy_detection_plots"
                    ),
                )
                if keep_raw == False:
...
def plot_match_overlap(
    match_files_directory, images_directory, output_directory="qc/matches/"
):
    ...
    fig, ax = plt.subplots(
        len(keys), 2, figsize=(10, fig_size_y), sharex="col", sharey=True
    )
    for i, v in enumerate(keys):
        ...
        ax[i][0].scatter(
            df_combined.xs(keys[i])["x1"],
            df_combined.xs(keys[i])["y1"],
            color="r",
            marker=".",
        )
        ax[i][1].scatter(
            df_combined.xs(keys[i])["x2"],
            df_combined.xs(keys[i])["y2"],
            color="r",
            marker=".",
        )
...
def pick_camera_locations(
    ...
    for i in image_file_paths:
        x, y, image_file_basename = hsfm.utils.pick_camera_location(
            i, center_lon, center_lat, dx=delta, dy=delta
        )

        df.loc[
            (df[image_file_column_name] == image_file_basename), longitude_column_name
        ] = x
        df.loc[
            (df[image_file_column_name] == image_file_basename), latitude_column_name
        ] = y
...
def run_metashape(
    ...
    bundle_adjusted_metadata_file = os.path.join(
        output_path, "bundle_adjusted_metadata.csv"
    )
    aligned_bundle_adjusted_metadata_file = os.path.join(
        output_path, "aligned_bundle_adjusted_metadata.csv"
    )

    if not isinstance(metashape_licence_file, type(None)):
        hsfm.metashape.authentication(metashape_licence_file)

    if isinstance(output_DEM_resolution, type(None)) and not isinstance(
        focal_length, type(None)
    ):
        print("No DEM output resolution specified.")
        print(
            "Using Ground Sample Distance from mean camera altitude above ground to estimate."
        )
        output_DEM_resolution = hsfm.core.estimate_DEM_resolution_from_GSD(
            images_metadata_file, pixel_pitch, focal_length
        )

        output_DEM_resolution = densecloud_quality * output_DEM_resolution
        print(
            "DEM resolution factored by densecloud quality setting:",
            output_DEM_resolution,
        )
    elif isinstance(output_DEM_resolution, type(None)) and isinstance(
        focal_length, type(None)
    ):
        print("No DEM output resolution specified. No focal length specified.")
        print(
            "Cannot compute GSD to estimate an optimal DEM resolution without a focal length."
        )
        print(
            "Setting output DEM resolution to 10 m. You can regrid the las file to a higher resolution as desired."
        )
        output_DEM_resolution = 10

    out = hsfm.metashape.images2las(
        project_name,
        images_path,
        images_metadata_file,
        output_path,
    ...
    ba_cameras_df, unaligned_cameras_df = hsfm.metashape.update_ba_camera_metadata(
        metashape_project_file, images_metadata_file
    )
    ba_cameras_df.to_csv(bundle_adjusted_metadata_file, index=False)

    x_offset, y_offset, z_offset = hsfm.core.compute_point_offsets(
        images_metadata_file, bundle_adjusted_metadata_file
    )

    ba_CE90, ba_LE90 = hsfm.geospatial.CE90(x_offset, y_offset), hsfm.geospatial.LE90(
        z_offset
    )
    ...
    if ba_CE90 > 0.01 or ba_LE90 > 0.01:
        ...
        if large_to_small_order == (reference_dem, dem):
            reference_dem = hsfm.utils.clip_reference_dem(
                dem,
                reference_dem,
        ...
        aligned_dem_file, transform = hsfm.asp.pc_align_p2p_sp2p(
            dem, reference_dem, output_path, verbose=verbose
        )
        ...
        x_offset, y_offset, z_offset = hsfm.core.compute_point_offsets(
            bundle_adjusted_metadata_file, aligned_bundle_adjusted_metadata_file
        )

        tr_ba_CE90, tr_ba_LE90 = (
        ...
        if dem_align_all:
            ...
            hsfm.utils.dem_align_custom(
                reference_dem, aligned_dem_file, dem_align_output_path, verbose=verbose
            )
        ...
        hsfm.utils.dem_align_custom(
            reference_dem, dem, dem_align_output_path, verbose=verbose
        )
        if generate_ortho:
            ortho_output_path, _, _ = hsfm.io.split_file(dem)

            hsfm.metashape.images2ortho(project_name, ortho_output_path)
...
def metaflow(
    ...
    if isinstance(focal_length, type(None)) and isinstance(
        camera_model_xml_file, type(None)
    ):
        try:
    ...
    if check_subsets:
        ...
        if len(subsets) > 1:
            ...
            for sub in subsets:
                ...
                tmp_df = ba_cameras_df[
                    ba_cameras_df["image_file_name"].isin(tmp)
            ...
            for sub_df in cameras_sub_clusters_dfs:
                ...
                sub_output_path = os.path.join(
                    output_path, "sub_cluster" + str(sub_counter)
                )
                p = pathlib.Path(sub_output_path)
                p.mkdir(parents=True, exist_ok=True)

                sub_images_metadata_file = os.path.join(
                    sub_output_path, "metashape_metadata.csv"
                )
                sub_df.to_csv(sub_images_metadata_file, index=False)

                hsfm.batch.metaflow(
                    project_name + "_sub_cluster" + str(sub_counter),
                    images_path,
        ...
        if len(unaligned_cameras_df) > 3:
            ...
            unaligned_output_path = os.path.join(output_path, "unaligned_subset")
            unaligned_images_metadata_file = os.path.join(
                output_path, "unaligned_subset", "metashape_metadata.csv"
            )
...
def batch_process(
    ...
    image_files = os.path.join(
        output_directory, "*", "*", "*", "*cropped_images", "*.tif"
    )
