...
def df_points_to_polygon_gdf(df, lon="lon", lat="lat", epsg_code="4326"):
    vertices = []
    ...
    polygon_gdf = gpd.GeoDataFrame(
        gpd.GeoSeries(polygon), columns=["geometry"], crs="epsg:" + epsg_code
    )
    return polygon_gdf


def df_xyz_coords_to_gdf(df, lon="lon", lat="lat", z="elevation", epsg_code="4326"):
    """
...
def df_xy_coords_to_gdf(df, lon="lon", lat="lat", epsg_code="4326"):
    """
...
def calculate_heading(point1_lon, point1_lat, point2_lon, point2_lat):
    ...
    y = math.cos(lat1) * math.sin(lat2) - (
        math.sin(lat1) * math.cos(lat2) * math.cos(delta_x)
    )
...
def rescale_geotif_to_file(geotif_file_name, scale_factor):
    ...
    output_file = os.path.join(
        file_path, file_name + "_sub" + str(scale_factor) + ".tif"
    )
...
def pick_points_from_basemap_tiles(tiles, utm_zone=None):
    ...
    base_map = (tiles * location).opts(
        opts.Points(width=500, height=500, size=12, color="black", tools=["hover"])
    )
...
def download_basemap_tiles_as_geotif(
    lon,
    lat,
    dx,
    dy,
...
def reproject_geotif(geotif_file_name, epsg_code, output_file_name=None, verbose=False):
    """
    ...
    if output_file_name == None:
        ...
        output_file_name = os.path.join(
            file_path, file_name + "_EPSG_" + str(epsg_code) + file_extension
        )
...
def compare_dem_extent(dem1_file, dem2_file):
    ...
    for lat, lon in zip(lats_list, lons_list):
        ...
        while c < 5:
            try:
                ...
                elevations.append(
                    result.json()["USGS_Elevation_Point_Query_Service"][
                        "Elevation_Query"
                    ]["Elevation"]
                )
                break
