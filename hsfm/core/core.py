...
def create_overlap_list(gcp_directory, image_directory, output_directory):
...
def create_overlap_list_from_match_files(
    match_files_directory, image_directory, output_directory, suffix=".match"
):
    ...
    for match_file in match_files:
        img1_fn, img2_fn = bare.core.parse_image_names_from_match_file_name(
            match_file, image_directory, "tif"
        )
...
def determine_flight_lines(
    ...
    df = hsfm.batch.calculate_heading_from_metadata(
        df,
...
def calculate_distance_principal_point_to_image_edge(
    focal_length_mm,
    image_width_px,
    image_height_px,
    ...
    elevation = hsfm.geospatial.sample_dem(
        [
            camera_lat_lon_wgs84_center_coordinates[1],
        ],
        [
    ...
    distance_pp_img_edge_x_m = (
        np.tan(np.deg2rad(angle_pp_img_edge_x)) * sensor_altitude_above_ground_m
    )
    distance_pp_img_edge_y_m = (
        np.tan(np.deg2rad(angle_pp_img_edge_y)) * sensor_altitude_above_ground_m
    )
...
def calculate_corner_coordinates(
    ...
    out = calculate_distance_principal_point_to_image_edge(
        focal_length_mm,
        image_width_px,
        image_height_px,
        camera_lat_lon_wgs84_center_coordinates,
    ...
    UL, UR, LR, LL = hsfm.trig.calculate_corner(
        camera_utm_lat, camera_utm_lon, half_width_m, half_height_m, heading
    )
    ...
    corner_elevations = hsfm.geospatial.sample_dem(
        corner_lons, corner_lats, reference_dem
    )

    return corner_lons, corner_lats, corner_elevations


def prep_and_generate_gcp(
    image_file_name,
    ...
    corner_lons, corner_lats, corner_elevations = calculate_corner_coordinates(
        camera_lat_lon_center_coordinates,
        reference_dem,
        focal_length_mm,
        image_width_px,
...
def subset_input_image_list(image_list, subset=None):
    ...
    if subset == None:
        ...
        image_list_df["image_index_number"] = (
            image_list_df["image_name"].str[11:].apply(int)
        )

        if isinstance(subset, tuple):
            image_list_df = image_list_df[
                image_list_df["image_index_number"].between(subset[0], subset[1])
            ]
        elif isinstance(subset, list):
            image_list_df = image_list_df[
                image_list_df["image_index_number"].isin(subset)
            ]
...
def slice_image_frame(grayscale_unit8_image_array, windows):
    ...
    left_slice = img_gray[
        window_left[0] : window_left[1], window_left[2] : window_left[3]
    ]
    top_slice = img_gray[window_top[0] : window_top[1], window_top[2] : window_top[3]]
    right_slice = img_gray[
        window_right[0] : window_right[1], window_right[2] : window_right[3]
    ]
    bottom_slice = img_gray[
        window_bottom[0] : window_bottom[1], window_bottom[2] : window_bottom[3]
    ]
...
def pick_fiducials_manually(
    ...
    while condition == True:
        ...
        principal_point, intersection_angle, fiducials = hsfm.utils.pick_fiducials(
            image_file_name
        )
        if intersection_angle > angle_max or intersection_angle < angle_min:
            print(
                "Intersection angle at principle point is not within orthogonality limits."
            )
            print("Try again.")
        else:
            if isinstance(image_array, np.ndarray):
...
def preprocess_image(
    ...
    if manually_pick_fiducials:
        if image_file_name:
            principal_point, intersection_angle, fiducials = pick_fiducials_manually(
        ...
        fiducials, principal_point = detect_fiducials_and_principal_point(
            windows, templates, img_gray, invisible_fiducial=invisible_fiducial
        )
        # QC routine
        ...
        if intersection_angle > angle_max or intersection_angle < angle_min:
            print(
                "Warning: intersection angle at principle point is not within orthogonality limits."
            )
                ...
                if intersection_angle > angle_max or intersection_angle < angle_min:
                    ...
                    if intersection_angle > angle_max or intersection_angle < angle_min:
                        ...
                        (
                            fiducials,
                            principal_point,
                        ) = detect_fiducials_and_principal_point(
                            windows,
                            templates,
                            img_gray,
                            noisify="bottom",
    ...
    if intersection_angle < angle_max and intersection_angle > angle_min:
        cropped = crop_about_principal_point(
            img_gray, principal_point, crop_from_pp_dist=crop_from_pp_dist
        )
...
def detect_fiducials_and_principal_point(
    windows,
    templates,
    ...
    fiducials = detect_fiducials(
        padded_slices, windows, templates, invisible_fiducial=invisible_fiducial
    )

    # detect principal point
    principal_point = determine_principal_point(
        fiducials[0], fiducials[1], fiducials[2], fiducials[3]
    )
...
def detect_fiducials(padded_slices, windows, templates, invisible_fiducial=None):
    ...
    left_fiducial = get_fiducial(
        left_slice_padded, left_template, window_left, position="left"
    )
    top_fiducial = get_fiducial(
        top_slice_padded, top_template, window_top, position="top"
    )
    right_fiducial = get_fiducial(
        right_slice_padded, right_template, window_right, position="right"
    )
    bottom_fiducial = get_fiducial(
        bottom_slice_padded, bottom_template, window_bottom, position="bottom"
    )
...
def determine_principal_point(
    left_fiducial, top_fiducial, right_fiducial, bottom_fiducial
):
    fa = np.array(
        [left_fiducial, top_fiducial, right_fiducial, bottom_fiducial], dtype=float
    )
    fx = fa[:, 0]
    fy = fa[:, 1]
...
def crop_about_principal_point(
    grayscale_unit8_image_array, principal_point, crop_from_pp_dist=11250
):
    img_gray = grayscale_unit8_image_array
...
def move_match_files_in_sequence(
    bundle_adjust_directory, image_prefix, stereo_directory, sequence
):
    i = sequence[0]
    j = i + 1
    ...
    match_files = sorted(
        glob.glob(os.path.join(bundle_adjust_directory, "*-clean.match"))
    )
...
def move_camera_files_in_sequence(
    bundle_adjust_directory, image_prefix, stereo_directory, sequence, extension=".tsai"
):
    i = sequence[0]
    j = i + 1
    ...
    camera_files = sorted(
        glob.glob(os.path.join(bundle_adjust_directory, "*" + extension))
    )
    ...
    new_camera_files = sorted(
        glob.glob(os.path.join(stereo_directory, "*" + extension))
    )
...
def metashape_cameras_to_tsai(
    project_file_path, original_metashape_metadata, image_extension=".tif"
):

    output_directory = os.path.join(
        os.path.dirname(project_file_path), "metashape_cameras"
    )
    hsfm.io.create_dir(output_directory)
    ...
    for i in unaligned_cameras:
        df[df["image_file_name"].str.contains(i)] = metashape_metadata_df[
            metashape_metadata_df["image_file_name"].str.contains(i)
        ]

    gdf = hsfm.geospatial.df_xyz_coords_to_gdf(
        df, lon="lon", lat="lat", z="alt", epsg_code="4326"
    )
    gdf = gdf.to_crs("epsg:4978")
...
def prepare_metashape_metadata(
    camera_positions_file_name,
    ...
    df.loc[~df["Altitude"].str.contains("unknown"), "alt"] = df.loc[
        ~df["Altitude"].str.contains("unknown")
    ]["Altitude"].values
...
def metadata_transform(metadata_file, pc_align_transform_file, output_file_name=None):
    ...
    for index, row in df.iterrows():
        C = [row.x, row.y, row.z]
        C_translation, R_transform = hsfm.core.extract_transform(
            pc_align_transform_file
        )
        row.x, row.y, row.z = hsfm.core.apply_position_transform(
            C, C_translation, R_transform
        )

        row = row.drop(["geometry"])
        df_tmp = df_tmp.append(row)

    transformed_metadata = hsfm.geospatial.df_xyz_coords_to_gdf(
        df_tmp, lon="x", lat="y", z="z", epsg_code="4978"
    )
    ...
    transformed_metadata = transformed_metadata.drop(
        ["x", "y", "z", "geometry"], axis=1
    )
    ...
    transformed_metadata = transformed_metadata.sort_values(
        by=["image_file_name"], ascending=True
    )
...
def compute_point_offsets(
    metadata_file_1, metadata_file_2, lon="lon", lat="lat", alt="alt"
):
    ...
    if len(df1) > len(df2):
        df1 = df1[
            df1["image_file_name"].isin(df2["image_file_name"].values)
        ].reset_index(drop=True)
    elif len(df1) < len(df2):
        df2 = df2[
            df2["image_file_name"].isin(df1["image_file_name"].values)
        ].reset_index(drop=True)

    epsg_code = hsfm.geospatial.lon_lat_to_utm_epsg_code(
        df2[lon].values[0], df2[lat].values[0]
    )
...
def determine_image_clusters(
    ...
    for i in gdf.polygon.values:
        d = gpd.GeoDataFrame(
            gpd.GeoSeries(i), columns=["geometry"], crs="epsg:" + epsg_code
        )
        footprints.append(d)
    ...
    while len(unmatched_files) > 0:
        ...
        gdf.loc[gdf["fileName"].isin(unmatched_files), "polygon"] = gdf.loc[
            gdf["fileName"].isin(unmatched_files), "geometry"
        ].buffer(radius_m)
    ...
    if qc:
        ...
        for i, v in enumerate(clusters):
            ...
            ax.annotate(s=str(i).zfill(3), xy=p, horizontalalignment="center", size=15)
...
def compute_square_footprint(
    gdf, image_square_dim, pixel_pitch, focal_length, flight_altitude_above_ground_m
):
    ...
    gdf["GSD"] = gdf["alt_above_ground"].apply(
        hsfm.core.compute_GSD, args=(pixel_pitch, focal_length, False)
    )
...
def estimate_DEM_resolution_from_GSD(
    images_metadata_file, pixel_pitch, focal_length, factor=3
):

    df = pd.read_csv(images_metadata_file)
    elevations = hsfm.geospatial.USGS_elevation_function(
