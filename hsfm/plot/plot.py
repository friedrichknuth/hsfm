...
def plot_image_histogram(
    image_array, image_base_name, output_directory="qc/image_histograms/", suffix=None
):
    ...
    n, bins, patches = ax.hist(
        img.ravel()[::40], bins=256, range=(0, 256), color="steelblue", edgecolor="none"
    )
    ...
    if output_directory == None:
        ...
        output_file_name = os.path.join(
            output_directory, image_base_name + suffix + ".png"
        )
...
def plot_principal_point_and_fiducial_locations(
    image_array,
    fiducials,
    principal_point,
    image_base_name,
    ...
    ax.scatter(top_fiducial[0], top_fiducial[1], s=0.2, color="midnightblue")
    ax.scatter(right_fiducial[0], right_fiducial[1], s=0.2, color="midnightblue")
    ax.scatter(bottom_fiducial[0], bottom_fiducial[1], s=0.2, color="midnightblue")
    ax.scatter(
        int(principal_point[0]),
        int(principal_point[1]),
        s=0.2,
        label="Principal Point",
        color="red",
    )
    ax.plot(
        [left_fiducial[0], right_fiducial[0]],
        [left_fiducial[1], right_fiducial[1]],
        color="k",
        lw=0.1,
    )
    ax.plot(
        [top_fiducial[0], bottom_fiducial[0]],
    ...
    if output_directory == None:
        ...
        output_file_name = os.path.join(
            output_directory, image_base_name + "_pp_and_fiducial_location.png"
        )
...
def plot_dem_difference_map(
    ...
    if spread == None:
        lowerbound, upperbound = np.nanpercentile(
            masked_array, [percentile_min, percentile_max]
        )
        spread = max([abs(lowerbound), abs(upperbound)])

    if not isinstance(ax, type(None)):
        im = ax.imshow(masked_array, cmap=cmap, clim=(-spread, spread), extent=extent)
...
def plot_dem_difference_from_file_name(
    dem_difference_file_name,
    output_file_name=None,
    cmap="RdBu",
    percentile_min=1,
    percentile_max=99,
    ...
    masked_array = hsfm.geospatial.mask_array_with_nan(array, nodata_value)
...
def plot_dem_with_hillshade(
    masked_array, output_file_name=None, cmap="inferno", clim=None
):
    """
...
def plot_dem_from_file(dem_file_name, output_file_name=None, cmap="inferno", clim=None):
    ...
    plot_dem_with_hillshade(
        masked_array, output_file_name=output_file_name, cmap=cmap, clim=clim
    )


def plot_intersection_angles_qc(
    intersections, file_names, show=False, expected_angle=90.0
):
    df = pd.DataFrame(
        {"Angle off mean": intersections, "filename": file_names}
    ).set_index("filename")
    df_mean = df - df.mean()
    fig, ax = plt.subplots(1, figsize=(10, 10))
    ...
    if show:
        plt.show()
    fig.savefig(
        "qc/image_preprocessing/principal_point_intersection_angle_off_mean.png"
    )
    plt.close()
    ...
    print(
        "Mean rotation off "
        + str(expected_angle)
        + " degree intersection at principal point:",
        angle,
    )
    print(
...
def plot_offsets(
    LE90, CE90, x_offset, y_offset, z_offset, title=None, plot_file_name=None
):
    x = x_offset
    y = y_offset
    z = z_offset
